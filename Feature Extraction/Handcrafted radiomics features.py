...
def loadSegArraywithID(fold,iden):
    ...
    segPath = [os.path.join(path,i) for i in pathList if ('roi' in i.lower()) & (iden in i.lower())][0]
...
for ind in range(len(dirlist)):
    ...
    mask = loadSegArraywithID(path,'roi')
    img = loadImgArraywithID(path,'im')
